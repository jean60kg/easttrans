...
public class ZxFileInfoController extends BaseController
{
    ...
    public AjaxResult uploadZxFile(HttpServletRequest request) throws IOException
    {
        ...
        if (!file.isEmpty())
        {
            ...
            System.out.print(RuoYiConfig.getUploadPath());
            String uploadFile = FileUploadUtils.upload(RuoYiConfig.getUploadPath(), file);
    ...
    public void downloadZxFileFlow(HttpServletRequest request, HttpServletResponse response)
    {
        ...
        if (zxFileInfo!=null)
        {
            FileUploadUtils.downloadByFlow(RuoYiConfig.getProfile()+zxFileInfo.getFilePath(),zxFileInfo.getFileName(), request, response);
        }
    }
